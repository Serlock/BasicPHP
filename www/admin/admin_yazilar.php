...
<body>
    ...
    <div class="content">
        <label style="font-size:16px">Tüm Yazılar | </label>

        <?php

        /*$q = "SELECT * FROM yazilar ";*/
        $q = "SELECT y.*, k.adi as kullanici_adi FROM yazilar as y, kullanicilar as k WHERE k.id = y.kullanici_id";
        ...
        if ($kullanicilar = $conn->query(($q))) {
            while ($row = mysqli_fetch_array($kullanicilar)) {
                ...
                echo $table_second =
                    ...
                    <td class='td_table'> $kullanici_adi</td>
                    <td class='td_table'><a href='yazi_sil.php?id=$kitap_id' style='text-decoration:underline'>Yazıyı Sil</a></td>
